...
public interface PatientRepository extends JpaRepository<Patient, Long> {
    ...
    @Query("select new com.tech.hospitalmanagement.dto.CPatientInfo(p.id,p.name) " + "from Patient p")
    List<CPatientInfo> getAllPatientsInfoConcrete();

    @Query("select new com.tech.hospitalmanagement.dto.BloodGroupStats(p.bloodGroup, " + "COUNT(p)) from Patient p group by p.bloodGroup order by COUNT(p)")
    ...
    @Query("SELECT p from Patient p LEFT JOIN FETCH p.appointments")  // this is understood by JPQL Query
